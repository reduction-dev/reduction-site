...
posts (one write for many reads) but it's great for processing events streams.
...
This is called a Write Ahead Log (WAL) because it contains writes that
happen before writing to a more permanent database structure, but it can be
helpful to think of the WAL as the foundation for DKV and the other structures
as layers of optimizations on top of the log.
...
DVK's use of cloud storage as the primary data store instead of local disks
separates a job's compute from its storage so that the clusters doesn't need to be
scaled to accommodate more data. This decision means that performing a write for
each record added to the WAL is infeasible due to per-request latency and cost.
Ultimately DKV relies on distributed checkpointing for recovery which makes
buffering the WAL and flushing it during the asynchronous phase of the
checkpoint process a natural approach.
...
and is often used to describe the in-memory representation of recently written
data. In most LSM databases memTables use
[Skip Lists](https://en.wikipedia.org/wiki/Skip_list) to store data. DKV uses
[Zip Trees](https://arxiv.org/abs/1806.06726) which have the same
performance characteristics but are easier for the author to understand. Zip
...
Using this property, Reduction can store state maps as multiple entries in DVK
with a shared key prefix.

## SSTables

Once the current MemTable reaches its size limit, DKV rotates the table, making
it immutable (a "sealed" memTable) and then flushing the table to
persistent storage.
...
naturally compacted as their zip trees are mutation and flushed to SSTables. And
then the compaction process continually organizes data within sorted runs
spanning SSTables.

DVK uses two separate compaction triggers for SSTables:
...
nodes start a checkpoint by requesting a new checkpoint from each DVK instance
...
The State Specs from Reduction jobs define how DKV will be updated and queried.
Consider this state spec for a Reduction Job:
...
:::note[Pseudo-values and pseudocode ahead]
...
// returned entries:
//   256|0|user-id|state-id|map-key-1|map-value-1
//   256|0|user-id|state-id|map-key-1|map-value-1
//   ...
```

Then when you set a value (`new-value`) for a key `map-key-1` in the map, DVK
...
parallel scans for each of their managed key groups. This is one of the many
reasons that the Reduction organizes the unbounded space of possible subject
keys into a finite set of key groups.
